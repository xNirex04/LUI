...
def Info(token):
    ...
    [{Fore.RED}Benutzer ID{Fore.RESET}]         {userID}
    [{Fore.RED}Erstellt am {Fore.RESET}]      {creation_date}
    [{Fore.RED}Sprachen{Fore.RESET}]        {language}
    ...
    [{Fore.RED}Telefonnummer{Fore.RESET}]    {phone if phone else ""}
    {Fore.RESET}{Fore.GREEN}<────────────Nitro Info─────────────>{Fore.RESET}
    [{Fore.RED}Nitro Status{Fore.RESET}]    {has_nitro}
    [{Fore.RED}Läuft ab in{Fore.RESET}]      {days_left if has_nitro else "0"} Tage
                ''')
